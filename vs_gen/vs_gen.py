...
DEFAULT_OUTPUT_TEMPLATES: Dict[str, str] = {
    ...
    "chunk_summary": "**{count}** new chunks were added."
}
...
def compile_player_prefix(player: str) -> re.Pattern:
    return re.compile(
        r"^player\s+" + re.escape(player) + r"[\s:,-]+",
        re.I
    )
...
def clean_death_message(
    message: str, player: str, custom_mapping: Dict[str, str]
) -> str:
    ...
    return (
        message
        if message and message.lower() != "died"
        else "unknown causes"
    )
...
def parse_timestamp(timestamp_str: str) -> Optional[datetime]:
    try:
        ...
        logger.error(
            f"Timestamp parsing error for '{timestamp_str}': {e}"
        )
        return None
...
class LogEvent:
    ...
    def from_line(line: str) -> Optional["LogEvent"]:
        ...
        event_type = (
            event_type_match.group(1).strip()
            if event_type_match
            else ""
        )
        end_index = line.find("]")
        message = (
            line[end_index + 1 :].strip() if end_index != -1 else ""
        )
...
def extract_player_name(text: str) -> Optional[str]:
    ...
    return (
        tokens[1]
        if first in ("player", "client") and len(tokens) > 1
        else tokens[0]
    )
...
class EventAggregator:
    def __init__(
        ...
        self.chunk_count: int = 0  # Count of new chunk save events
        ...
        if event.event_type == "Audit":
            if is_interesting_audit_event(event.message) or (
                ...
                and any(
                    ...
                    for word in ["moved", "took", "put"]
                )
            ):
                self._process_audit_event(event, lower_message)
        elif (
            event.event_type == "Notification"
            and "temporal storm" in lower_message
        ):
            self.storm_events.append(event)
        # Count new chunk save events by parsing the numeric value from the log message.
        if "chunk" in lower_message and "saved" in lower_message:
            chunk_match = re.search(r"(\d+)\s*x?\s*chunk", lower_message)
            if chunk_match:
                try:
    ...
    def _process_audit_event(
        self, event: LogEvent, lower_message: str
    ) -> None:
        ...
        if "died" in lower_message or "death message" in lower_message:
            death_match = re.search(
                r"death message:\s*(.*)", event.message, re.I
            )
            cause_text = (
                death_match.group(1).strip()
                if death_match
                else event.message
            )
            cause_clean = clean_death_message(
                cause_text, player, self.custom_message_map
            )
            self.players[player_lower].death_details.append(cause_clean)
        # Process kill events
        if "killed" in lower_message:
            kill_match = re.search(
                r"killed\s+(?!by)(?:a\s+|an\s+)?([\w:.-]+)",
                event.message,
                re.I
            )
            if kill_match:
                ...
                enemy_mapped = apply_custom_mapping(
                    enemy_mapped, self.custom_message_map
                )
                self.players[player_lower].kills.append(enemy_mapped)
        # Process metal movement events
        if any(word in lower_message for word in ["moved", "took", "put"]):
            for metal in self.metals:
                if metal in lower_message:
                    ...
                    if (
                        quantities
                        and max(int(q) for q in quantities)
                        >= self.large_metal_threshold
                    ):
                        self.players[player_lower].large_metals.add(metal)

    def aggregate_storm_times(
        self,
    ) -> Tuple[
        Optional[datetime],
        Optional[datetime],
    ...
    def get_complete_storms(self) -> List[Tuple[datetime, datetime]]:
        ...
        sorted_events = sorted(
            self.storm_events, key=lambda e: e.timestamp
        )
        for event in sorted_events:
            ...
            if "temporal storm is imminent" in msg_lower:
                ...
                current_storm_start
                and "temporal storm seems to be waning" in msg_lower
            ):
...
class NarrativeGenerator:
        ...
        for player, data in sorted(players.items()):
            ...
            if self.brief:
                if death_count == 0 and kill_count == 0:
                    ...
                    story = (
                        f"**{player.capitalize()}**: {death_count} death(s), "
                        f"{kill_count} kill(s)."
                    )
            else:
                # Detailed narrative generation (existing logic)
                if death_count == 1:
                    death_phrase = self.templates["single_death"].format(
                        causes=grammatical_join(data.death_details)
                    )
                elif death_count == 2:
                    death_phrase = self.templates["double_death"].format(
                        causes=grammatical_join(data.death_details)
                    )
                elif death_count >= 3:
                    top_deaths, _ = self._summarize_top(data.death_details)
                    death_phrase = self.templates["multiple_deaths"].format(
                        count=death_count, causes=top_deaths
                    )
                else:
                    death_phrase = ""
                if kill_count == 1:
                    kill_phrase = self.templates["single_kill"].format(
                        kills=grammatical_join(data.kills)
                    )
                elif kill_count == 2:
                    kill_phrase = self.templates["double_kill"].format(
                        kills=grammatical_join(data.kills)
                    )
                elif kill_count >= 3:
                    top_kills, _ = self._summarize_top(data.kills)
                    kill_phrase = self.templates["multiple_kills"].format(
                        count=kill_count, kills=top_kills
                    )
                else:
                    kill_phrase = ""
                metal_phrase = ""
                if data.large_metals:
                    metal_phrase = (
                        "They moved a large sum of "
                        + grammatical_join(
                            sorted(list(data.large_metals))
                        )
                    )
                phrases = [
                    phrase
                    for phrase in (death_phrase, kill_phrase)
                    if phrase
                ]
                if phrases:
                    ...
                    if metal_phrase:
                        phrases_metal = [
                            phrase
                            for phrase in (combined_phrases, metal_phrase)
                            if phrase
                        ]
                        combined_phrases = ". ".join(phrases_metal)
                    story = self.templates["player_story"].format(
    ...
    def generate_story(self, players: Dict[str, PlayerData]) -> str:
        ...
        if player_summaries:
            return "\n".join(
                player["story"] for player in player_summaries
            )
        else:
            return self.templates.get("no_events", "Nothing ever happens")

    def generate_storm_summary(
        self,
        storms: List[Tuple[datetime, datetime]],
        fallback: Tuple[
            Optional[datetime],
            Optional[datetime],
            Optional[datetime]
        ...
        if storms and len(storms) > 1:
            n = len(storms)
            total_duration = sum(
                (waning - imminent).total_seconds() / 60
                for (imminent, waning) in storms
            )
            avg_duration = round(total_duration / n)
            return (
                f"{n} temporal storms occured, with an average duration of "
            ...
            return self.templates["storm_occurred"].format(
                start=imminent.strftime(TIMEONLY_FORMAT),
                duration=duration
            )
        else:
            ...
            if storm_start and storm_waning:
                duration = int(
                    (storm_waning - storm_start).total_seconds() / 60
                )
                return self.templates["storm_occurred"].format(
                    start=storm_start.strftime(TIMEONLY_FORMAT),
                    duration=duration
                )
            elif storm_start:
                return self.templates["storm_start"]
            elif storm_soon:
                return self.templates["storm_soon"].format(
                    start=storm_soon.strftime(TIMEONLY_FORMAT)
                )
            elif storm_waning:
                return self.templates["storm_ending"].format(
                    end=storm_waning.strftime(TIMEONLY_FORMAT)
                )
            return ""
    ...
    if config_file:
        try:
            with open(config_file, "r", encoding="utf-8") as f:
                config = json.load(f)
            custom_message_map.update(
                config.get("custom_message_map", {})
            )
            custom_enemy_map.update(
                config.get("custom_enemy_map", {})
            )
            output_templates.update(
                config.get("output_templates", {})
            )
            logger.info(
                "Configuration loaded successfully from %s", config_file
            )
        except Exception as e:
            logger.error(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Parse game logs and generate a narrative summary."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-file", "-o", default="summary.txt",
        help="Output summary file"
    )
    parser.add_argument(
        "--state", "-s", default="last_timestamp.txt",
        help="State file for last timestamp"
    )
    parser.add_argument(
        "--recursive", "-r", action="store_true",
        help="Recursively search for log files in subdirectories"
    )
    parser.add_argument(
        "--char-limit", "-c", type=int, default=0,
        help="Maximum character limit for output message (0 for no limit)"
    )
    parser.add_argument(
        "--config", "-C",
        help=(
            "Optional JSON configuration file for custom mappings and "
            "templates"
        )
    )
    parser.add_argument(
        "--force-full", "-f", action="store_true",
        help="Force a full search ignoring the state file"
    )
    parser.add_argument(
        "--delete-state", "-d", action="store_true",
        help="Delete the state file for a reset before processing"
    )
    parser.add_argument(
        "--time-range", "-t",
        help=(
            "Force search of events within a set time period from the last "
            "message's timestamp (e.g., '30m', '2h', '1d')"
        )
    )
    parser.add_argument(
    ...
    custom_message_map, custom_enemy_map, output_templates = load_config(
        args.config
    )

    # If delete-state flag is set, remove the state file before processing
    if args.delete_state:
        ...
        # Read state file to determine last processed timestamp; process all
        # messages if missing or invalid.
        if os.path.exists(args.state):
            try:
                with open(args.state, "r", encoding="utf-8") as f:
                    state_content = f.read().strip()
                    last_timestamp = parse_timestamp(state_content)
                    if last_timestamp is None:
                        raise ValueError("Invalid timestamp in state file.")
            except Exception as e:
                logger.warning(
                    "State file issue: %s. Processing all messages.", e
                )
                last_timestamp = datetime.min
        else:
    ...
    # If force-full flag is set, override last_timestamp to process all events.
    if args.force_full:
        logger.info(
            "Force full search enabled; ignoring state file timestamp."
        )
        last_timestamp = datetime.min
    ...
    # If time-range option is provided, pre-scan log files to determine the
    # maximum timestamp and then adjust last_timestamp accordingly.
    if args.time_range:
        time_delta = parse_time_range(args.time_range)
        if time_delta is None:
            logger.warning(
                "Invalid time range specified. Ignoring --time-range option."
            )
        else:
            max_ts = datetime.min
            for log_file in log_files:
                try:
                    ...
                    logger.error(
                        "Error processing file %s during time range: %s",
                        log_file,
                        e
                    )
            if max_ts == datetime.min:
                logger.warning(
                    "No valid timestamps found in logs for time range"
                    "Ignoring --time-range option."
                )
            else:
                new_lower_bound = max_ts - time_delta
                logger.info(
                    "Time range option active: processing events from %s "
                    "(max timestamp %s minus %s)",
                    new_lower_bound.strftime(DATE_FORMAT),
                    max_ts.strftime(DATE_FORMAT),
                    args.time_range
                )
                last_timestamp = new_lower_bound

    aggregator = EventAggregator(
        custom_message_map, custom_enemy_map, DEFAULT_METALS,
        DEFAULT_LARGE_METAL_THRESHOLD
    )
    ...
    player_story = narrative_gen.generate_story(
        aggregator.get_player_data()
    )
    # Process storm events: obtain list of complete storms and fallback
    # aggregated times.
    complete_storms = aggregator.get_complete_storms()
    fallback_storms = aggregator.aggregate_storm_times()
    storm_summary = narrative_gen.generate_storm_summary(
        complete_storms, fallback_storms
    )
    chunk_summary = narrative_gen.generate_chunk_summary(
        aggregator.chunk_count
    )
    # Build the complete summary parts list.
    parts = []
    # If time-range is set, add a message at the top.
    if args.time_range:
        match = re.match(
            r"(\d+)([mhd])$", args.time_range.strip().lower()
        )
        if match:
            value = int(match.group(1))
            unit = match.group(2)
    ...
    if (
        args.char_limit and args.char_limit > 0
        and len(full_summary) > args.char_limit
    ):
        full_summary = (
            full_summary[: args.char_limit - 3] + "..."
            if args.char_limit > 3
            else full_summary[: args.char_limit]
        )

    # Write output and update state file with the latest timestamp
    try:
        with open(
        ...
        with open(
            args.state, "w", encoding="utf-8"
        ) as state_file:
            state_file.write(max_timestamp_in_run.strftime(DATE_FORMAT))
